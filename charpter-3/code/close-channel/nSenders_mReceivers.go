...
// 这是最复杂的一种情形。我们不能让接收者和发送者中的任何一个关闭用来传输数据的通道，
// 我们也不能让多个接收者之一关闭一个额外的信号通道。
// 这两种做法都违反了通道关闭原则。
// 然而，我们可以引入一个中间调解者角色并让其关闭额外的信号通道来通知所有的接收者和发送者结束工作。
// 具体实现见下例。注意其中使用了一个尝试发送操作来向中间调解者发送信号。

// 这里通过关闭协调通道来释放datach资源，而不是直接关闭datach
func main() {
	...
	// toStop是一个用来通知中间调解者让其，关闭信号通道stopCh的第二个信号通道。
	// 此第二个信号通道的发送者为dataCh数据，通道的所有的发送者和接收者，
	// 它的接收者为中间调解者。它必须为一个缓冲通道。
	// 在此例中，通道关闭原则依旧得到了遵守。
	// 请注意，信号通道toStop的容量必须至少为1。
	// 如果它的容量为0，则在中间调解者还未准备好的情况下就已经有某个协程向toStop发送信号时，此信号将被抛弃。
	toStop := make(chan string, 1)
	/*
	我们也可以不使用尝试发送操作向中间调解者发送信号，
	但信号通道toStop的容量必须至少为数据发送者和数据接收者的数量之和，
